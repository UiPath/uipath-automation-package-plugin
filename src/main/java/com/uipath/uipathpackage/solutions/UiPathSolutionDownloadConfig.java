...
import com.uipath.uipathpackage.models.solutions.SolutionDownloadConfigOptions;
...
public class UiPathSolutionDownloadConfig extends Recorder implements SimpleBuildStep {
    ...
    public UiPathSolutionDownloadConfig(String name,
    ...
    public void perform(Run<?, ?> run, FilePath workspace, EnvVars env, Launcher launcher, TaskListener listener)
        ...
        if (tempRemoteDir == null) {
            throw new AbortException("Failed to create temp folder for solution config download.");
        }
        ...
        try {
            ...
            SolutionDownloadConfigOptions options = new SolutionDownloadConfigOptions();
            ...
            ConfigFormat effectiveFormat = (format == null) ? ConfigFormat.yaml : format;
            options.setFormat(effectiveFormat);
            ...
            if (exitCode != 0) {
                throw new AbortException("UiPath CLI solution download-config failed with exit code: " + exitCode);
            }
            logger.println("Download solution config completed successfully.");
    ...
    @Symbol("UiPathSolutionDownloadConfig")
    @Extension
    public static final class DescriptorImpl extends BuildStepDescriptor<Publisher> {
        ...
        public ListBoxModel doFillFormatItems(@AncestorInPath Item item) {
            ...
            for (ConfigFormat cf : ConfigFormat.values()) {
                m.add(cf.toString(), cf.name());
            }
            return m;
        }
    }
