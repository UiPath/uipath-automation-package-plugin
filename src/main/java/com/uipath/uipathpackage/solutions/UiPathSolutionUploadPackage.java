...
public class UiPathSolutionUploadPackage extends Recorder implements SimpleBuildStep {
    ...
    public void perform(Run<?, ?> run, FilePath workspace, EnvVars env, Launcher launcher, TaskListener listener)
        ...
        try {
            ...
            FilePath expandedSolutionPackagePath = solutionPackagePath.contains("${WORKSPACE}") ?
                    new FilePath(launcher.getChannel(), envVars.expand(solutionPackagePath)) :
                    workspace.child(envVars.expand(solutionPackagePath));
    ...
    private void validateParameters() throws AbortException {
        Utility util = new Utility();
        util.validateParams(solutionPackagePath, "Invalid solution path");
     ...
     * {@link Publisher}.
     */
    @Symbol("UiPathSolutionUploadPackage")
    @Extension
    public static final class DescriptorImpl extends BuildStepDescriptor<Publisher> {
        ...
        public FormValidation doCheckSolutionPath(@QueryParameter String value) {
            if (value == null || value.trim().isEmpty()) {
                return FormValidation.error("Solution path is required.");
            }
